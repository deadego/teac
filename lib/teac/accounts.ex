defmodule Teac.Accounts do
  import Ecto.Query
  import Ecto.Changeset

  alias Teac.Repo
  alias Teac.Accounts.{User, Identity}
  ...
  def admin?(%User{} = user) do
    user.email in Teac.config([:admin_emails])
  end
  ...
  def update_public_settings(%User{} = user, attrs) do
    ...
    |> case do
      {:ok, new_user} ->
        # Teac.execute(__MODULE__, %Events.PublicSettingsChanged{user: new_user})
        {:ok, new_user}
  ...
  """
  def register_twitch_user(primary_email, info, emails, token) do
    if user = get_user_by_provider(:twitch, primary_email) do
      update_twitch_token(user, token)
    else
      info
      |> User.twitch_registration_changeset(primary_email, emails, token)
      |> Repo.insert()
    end
  end

  def get_user_by_provider(provider, email) when provider in [:twitch] do
    query =
      from(u in User,
  ...
  defp update_twitch_token(%User{} = user, new_token) do
    identity =
      Repo.one!(from(i in Identity, where: i.user_id == ^user.id and i.provider == "twitch"))
